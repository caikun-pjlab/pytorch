...
def compare_dict_tensors(dict_base, dict_control, precision):
    if len(set(dict_base.keys())) != len(set(dict_control.keys())):
        logger.info(
            "Mismatch parameter name. pt2.keys(): %s and pt2_optimus.keys(): %s",
            dict_base.keys(),
            dict_control.keys(),
        )
        return False
    ...
    for key in dict_base.keys():
        ...
        if not torch.allclose(
            ...
            print_graph(dict_base[key], "value pt2.")
            print_graph(dict_control[key], "value in pt2-optimus.")
            is_allclose = False
    return is_allclose
...
def compare_tuple_tensors(tuple_base, tuple_control, precision):
    ...
    for i in range(len(tuple_base)):
        ...
        if not torch.allclose(
            ...
            print_graph(tuple_base[i], "forward output in pt2.")
            print_graph(tuple_control[i], "forward output in pt2-optimus.")
            is_allclose = False
    return is_allclose
...
def run_model(model_base, model_control, model_input, num_iteration=10, precision=1e-4):
    clean_memory()
